...
class SystemMetrics(tk.Canvas):
    def __init__(self, parent):
        # Dynamically calculate based on parent size
        parent_width = parent.winfo_reqwidth() or 1440  # fallback size if not yet drawn
        ...
        user_icon = Image.open(user_icon_path).resize((32, 32))
    ...
    def embed_metrics_frame(self):
        ...
        tk.Label(frame, image=self.user_icon_img, bg='white').place(x=self.width - 56, y=18)

        self.draw_metric(frame, "110 Psi", "#F58F8F", "#FFD3D3", pressure, 155, 20, 80)
        self.draw_metric(frame, "27 °C", "#5B93F5", "#A9D0FF", temperature, 50, 20, 150)

        tk.Label(frame, text="Leak Rate:", font=('Poppins', 14, 'bold'), bg='white').place(x=60, y=230)
        tk.Label(frame, text="2.1 x 10⁻⁹", font=('Poppins', 14, 'bold'), fg='green', bg='white').place(x=70, y=265)

        tk.Label(frame, text="Status:", font=('Poppins', 14, 'bold'), bg='white').place(x=250, y=230)
        tk.Label(frame, text="OK", font=('Poppins', 16, 'bold'), fg='green', bg='white').place(x=330, y=230)
        tk.Label(frame, image=self.check_icon_img, bg='white').place(x=self.width-130, y=270)



    def draw_metric(self, parent, label_text, label_color, bar_color, value, max_value, x, y):
        label_width = 92
        bar_width = self.width - label_width - 51  # 51 = divider + margin adjustment
        height = 50
        radius = 25
...
class DashboardPage(tk.Frame):
    def __init__(self, master):
        ...
        self.sidebar_width = int(0.2 * self.window_width)  # 20% sidebar
